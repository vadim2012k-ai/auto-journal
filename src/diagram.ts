...
import type { Car, ZoneId } from './types.js';
...
function axleStub(fromX: number, fromY: number, toX: number, toY: number): string {
  return `<line x1="${fromX}" y1="${fromY}" x2="${toX}" y2="${toY}" stroke="#475569" stroke-width="5" stroke-linecap="round" stroke-dasharray="2 8"/>`;
}

function cabinBlock(cabinFill: string, y: number, height: number): string {
 ...
 */
function drivetrainBoxes(car: Car, gearboxFill: string): string {
  const driveType = car.driveType;
  const gearboxBox = `
...
export function renderCarDiagram(car: Car): string {
...
export function zoneLabel(zoneId: ZoneId): string {
